"""ds003458 SCP x reward probability cross-correlation analysis.

Tests whether Slow Cortical Potentials (0.01-0.1 Hz) at FC5 correlate
with trial-by-trial reward probability. SCP band matches the stimulus
timescale (~0.03 Hz) unlike classic delta (1-4 Hz) which is 30-100x
too fast.

Pre-registration: results/ds003458_preregistration.md (Amendment 2)

Run:
    python -m neurophase.experiments.ds003458_scp_analysis
"""
...
PRIMARY_CHANNEL = "FC5"
SCP_BAND: tuple[float, float] = (0.01, 0.1)


def _build_reward_slow(
    ...
    """Build reward probability at SCP timescale."""
    # Interpolate to EEG rate
    reward_cont = interpolate_to_eeg_rate(reward_prob, trial_onsets, fs, total_dur)
    reward_cont = reward_cont[:n_eeg]

    # Bandpass same as SCP (0.01-0.1 Hz)
    nyq = fs / 2.0
    low = SCP_BAND[0] / nyq
    high = SCP_BAND[1] / nyq
    b, a = butter(2, [low, high], btype="bandpass")
    filtered = filtfilt(b, a, reward_cont).astype(np.float64)

    # Z-score
    m, s = float(np.mean(filtered)), float(np.std(filtered))
    result: np.ndarray = ((filtered - m) / s if s > 0 else filtered - m).astype(np.float64)
    return result


def run_scp_analysis(
    data_root: str | Path = "data/ds003458",
    n_surrogates: int = 1000,
    seed: int = 42,
) -> dict[str, Any]:
    """Run per-subject SCP x reward xcorr analysis."""
    ...
    for sid in subjects:
        ...
        try:
            ...
            # Extract SCP
            scp = extract_scp(ch_data, fs=fs, band=SCP_BAND, smooth_s=10.0, channel_name=ch)

            # Build reward at SCP timescale
            ...
            # Trim edges (5%)
            n_trim = int(0.05 * min(len(scp.signal), len(reward_slow)))
            scp_trimmed = scp.signal[n_trim:-n_trim] if n_trim > 0 else scp.signal
            rew_trimmed = reward_slow[n_trim:-n_trim] if n_trim > 0 else reward_slow

            # Held-out: last 30%
            n_ho = int(0.30 * len(scp_trimmed))
            scp_ho = scp_trimmed[-n_ho:]
            rew_ho = rew_trimmed[-n_ho:]
            ...
            print(f"FAILED: {e}")
            per_subject.append({"subject": sid, "max_xcorr": None, "error": str(e)})
    ...
    return {
        "experiment": "ds003458_scp_xcorr",
        "timestamp": datetime.now(UTC).isoformat(),
        "dataset": "OpenNeuro ds003458 v1.1.0",
        "preregistration": "results/ds003458_preregistration.md (Amendment 2)",
...
def save_results(results: dict[str, Any], output_dir: str | Path = "results") -> Path:
    ...
    path = out_dir / f"ds003458_scp_xcorr_{date_str}.json"
...
def main() -> None:
    """Entry point."""
    print("=" * 60)
    print("  NEUROPHASE · ds003458 SCP (0.01-0.1 Hz) x Reward")
    print("=" * 60)
    print()

    results = run_scp_analysis()
