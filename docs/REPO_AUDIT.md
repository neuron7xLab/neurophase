...
| Gap | Status | Note |
|---|---|---|
| Branch-protection rules on `main` | **config-level — cannot be set from repo content alone** | Must be configured in GitHub settings: require CI green, require 1 review (from CODEOWNERS), require linear history, require signed commits if desired, disallow force-push. |
| Actions pinned by SHA | by `@v4`/`@v5` tags | Dependabot now manages; pinning by SHA is possible but adds friction; defer unless a concrete supply-chain signal appears. |
...
| OSSF Scorecard / SLSA provenance | not adopted | Appropriate for project maturity beyond v1.2; revisit at v2. |
| Coverage badge (codecov or equivalent) | not adopted | `pytest --cov` already runs in CI; trend tracking not yet published externally. |
